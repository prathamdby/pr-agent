...
const bridgeAbortMock = vi.hoisted(() => vi.fn(async () => undefined));
...
describe("cursorAgentRunnerProvider", () => {
  ...
  it("aborts active sends and the MCP bridge once without disposing either resource", async () => {
      ...
      async (_model, _context, options) =>
        new Promise<never>((_resolve, reject) => {
          ...
          options?.signal?.addEventListener(
            "abort",
            () => reject(new Error("active send aborted")),
            { once: true },
          );
        }),
    );
    ...
    await expect(send).rejects.toThrow("active send aborted");

    expect(activeSignal?.aborted).toBe(true);
    expect(bridgeAbortMock).toHaveBeenCalledTimes(1);
    expect(agentDispose).not.toHaveBeenCalled();
    expect(bridgeDisposeMock).not.toHaveBeenCalled();
  });
