...
function makeOctokitStub(overrides: Partial<{
	...
	return {
		rest: {
			reactions: {
				...
				createForPullRequestReviewComment: overrides.createForPullRequestReviewComment ?? vi.fn().mockResolvedValue({}),
			},
			issues: { createComment: overrides.createComment ?? vi.fn().mockResolvedValue({}) },
			pulls: {
				createReplyForReviewComment: overrides.createReplyForReviewComment ?? vi.fn().mockResolvedValue({}),
				get: overrides.pullsGet ?? vi.fn().mockResolvedValue({ data: { head: { sha: "deadbeef" } } }),
			},
		},
