...
import { REVIEW_COMPACT_SHARED_METHODOLOGY_ENABLED } from "../../settings/reviewConstants.js";

/** Correctness specialist prompt: investigation methodology plus a structured findings report. */
export function buildAutomatedSystemPrompt(options?: {
  readonly compactSharedMethodology?: boolean;
}): string {
  const compactSharedMethodology =
    options?.compactSharedMethodology ?? REVIEW_COMPACT_SHARED_METHODOLOGY_ENABLED;
  const investigationMethod = compactSharedMethodology
    ? [...compactInvestigationMethod]
    : [
  ...
  const bugPatterns = compactSharedMethodology
    ? [...compactBugPatternCatalogue]
    : [
  ...
  const reportingGate = compactSharedMethodology
    ? [...compactReportingGate]
    : [
        "## Reporting gate",
