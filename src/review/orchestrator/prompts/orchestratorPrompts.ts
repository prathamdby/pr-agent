...
export const orchestratorSystemPrompt = [
  ...
  "Specialists are Deep Reviewers: they report objective defects only. Product tradeoffs, architecture placement, and risk-acceptance decisions are judgment calls for a human, not findings.",
  ...
  "During synthesis, derive the review from accepted placements and publish one final summary through `publish_summary`. List remaining human decisions in `judgmentCalls`.",
...
export function renderJudgmentTurn(outcome: ReportOutcome): string {
  return [
    ...
    "Drop product or architecture tradeoffs that need a human decision. Those are judgment calls, not findings.",
...
export function renderSynthesisTurn(params: {
  ...
  const briefLines =
    params.prIntent != null || params.architectureNotes != null
      ? [
          "",
          "<recon_brief>",
          JSON.stringify(
            {
              prIntent: params.prIntent ?? "",
  ...
  return [
    ...
    "Write judgmentCalls for product or architecture decisions that still need a human. Do not restate findings. Leave the array empty when the change is mechanical.",
