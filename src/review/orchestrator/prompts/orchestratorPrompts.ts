...
export const orchestratorSystemPrompt = [
  ...
  "During judgment, re-apply the causal-publication contract independently. Specialist reports are evidence, never authority. You may re-read the checkout through execute cells; a reserved publish round survives those re-reads, so evidence gathering can never starve the terminal call. Publish only findings that meet that contract through the active `publish_thread` tool.",
...
export function renderJudgmentTurn(outcome: ReportOutcome, ledger: FindingLedger): string {
  ...
  return [
    ...
    ...(slimmed.count > 0
      ...
      : []),
    "Call `publish_thread` exactly once with every worthy remaining finding, using the reserved publish round. One call with zero findings is valid when none survive judgment. Do not publish a summary in this turn.",
    "",
    "<specialist_report>",
