...
import { summarizeCiFacts } from "../../review/ci/analyzeCi.js";
import {
 ...
 * workflow_run / check_suite completed. Reads `pr_head_ci_state`. Does not
 * snapshot GitHub or run the CI author.
 */
export async function executeCiRefreshJob(
  ...
  const row = await loadPrHeadCiState(pool, data.owner, data.repo, data.headSha);
  if (row == null) {
    logDebug("ci_refresh_skipped_no_state", {
      owner: data.owner,
      repo: data.repo,
      pr: data.prNumber,
  ...
  const ciSummary = summarizeCiFacts(row.checks);
