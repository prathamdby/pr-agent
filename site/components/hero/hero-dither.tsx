/**
 * Dithered dot field behind the hero. Copied from the kimaki website
 * (github.com/remorses/kimaki, website/src/components/hero-dither.tsx), which ports
 * DitherContent from mintlify.com/data. Two edits: colour scheme comes from
 * prefers-color-scheme instead of a .dark class, and the accent reads --accent.
 */
...
export function HeroDither({ offsetX = 0 }: { offsetX?: number }) {
  ...
  useEffect(() => {
    ...
    const edgeY = (y: number) => {
      ...
      if (fade <= 0) return 1;
      return Math.max(0, Math.min(Math.min(1, y / fade), Math.min(1, (height - y) / fade)));
    };
    const edgeX = (x: number) => {
      const fade = Math.min(48, width / 8);
      if (fade <= 0) return 1;
      return Math.max(0, Math.min(Math.min(1, x / fade), Math.min(1, (width - x) / fade)));
    };
    ...
    const draw = (now: number) => {
      ...
      for (const dot of dots) {
        ...
        const lit = Math.max(dot.isGreen ? 1 : 0, pulsing ? 1 : 0, rippleStrength);
    ...
    const layout = () => {
      ...
      for (let x = PITCH / 2; x < width; x += PITCH) {
        for (let y = PITCH / 2; y < height; y += PITCH) {
          ...
          const ny = y / SCALE;
          if (
            0.65 * valueNoise(nx, ny) + 0.35 * valueNoise(2.3 * nx + 11.7, 2.3 * ny + 5.1) <
            THRESHOLD
          ) {
            continue;
          }
    ...
    const onClick = (event: MouseEvent) => {
      ...
      if (
        ...
        target.closest('a, button, input, select, textarea, label, [role="button"], [role="link"]')
      ) {
        return;
      }
      const now = performance.now();
