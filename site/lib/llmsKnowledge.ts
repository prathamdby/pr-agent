...
export const KNOWLEDGE_CHUNKS: readonly KnowledgeChunk[] = [
  ...
  },
  {
    ...
    body: lines([
      ...
      "check_run and status write pr_head_ci_state and enqueue a debounced ci-projection job. The worker consumes that queue and renders CI cells from the row. Ask reads a ci_state block from the same row. The triage report stamps a rollup marker the projector patches, including after a push whose work-item head is still the pre-push SHA. The own check is excluded by App id or work-item external id, not by name. A failing rollup authors once per facts hash and stores the result on authored. Leftover ci-refresh jobs become one projection. PostHog work completed carries ci_rollup, ci_failing_count, and ci_authored. One ci state changed event fires when rollup moves.",
