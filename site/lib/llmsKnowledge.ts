...
export const KNOWLEDGE_CHUNKS: readonly KnowledgeChunk[] = [
  ...
  },
  {
    ...
    body: lines([
      ...
      "/ask … or mention the App bot ({slug}[bot]): answer a code question in the same thread. The prompt includes a read-only ci_state block from pr_head_ci_state. No CI tool is added. The literal string @bot only matches if the App was named that.",
  ...
  },
  {
    ...
    body: lines([
      ...
      "Subscribe to pull_request, issue_comment, pull_request_review_comment, workflow_run, check_suite, check_run, and status.",
  ...
  },
  {
    ...
    body: lines([
      ...
      "check_run and status write pr_head_ci_state and enqueue a debounced ci-projection job. workflow_run and check_suite completed deliveries enqueue the same projection without writing facts. pull_request opened, synchronize, and reopened enqueue that job when the head row is missing or seeded_at is null. Ack, ticks, and publish do the same after they write the comment. The worker consumes that queue and renders CI cells from the row. An incomplete seed stays unavailable on the cell and the rollup marker. A row whose seeded_at is still null is seeded on the next projection. Cancelled check conclusions roll up as failing. Each projection refreshes the head-to-PR list. A terminal review with an open check is closed from the recorded outcome. Ask reads a ci_state block from the same row. The triage report stamps a rollup marker the projector patches, including after a push whose work-item head is still the pre-push SHA. The own check is excluded by App id or work-item external id, not by name. A failing rollup authors once per facts hash and stores the result on authored. PostHog work completed carries ci_rollup, ci_failing_count, and ci_authored. One ci state changed event fires when rollup moves.",
