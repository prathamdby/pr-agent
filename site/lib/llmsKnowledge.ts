...
export const KNOWLEDGE_CHUNKS: readonly KnowledgeChunk[] = [
  ...
  },
  {
    ...
    body: lines([
      ...
      "/review force: cancel any queued or running review and start a new one on the latest commit.",
      "/describe: write summary bullets and an optional diagram into the PR body.",
      "/ask … or mention the App bot ({slug}[bot]): answer a code question in the same thread. The literal string @bot only matches if the App was named that.",
      "/triage: apply-mode fix for open bot findings. /triage preview renders would-be diffs without push. /triage all replays those stored hunks (refused without a matching preview on this head).",
  ...
  },
  {
    ...
    body: lines([
      ...
      `${FEATURE_KEYS[2]}: off | manual | auto. Default auto. Rechecks open findings on every synchronize push or /verify. That default spends tokens on every push. Terminal failure edits the CI cell or one stub line.`,
  ...
  },
  {
    ...
    body: lines([
      ...
      "Compose postgres is not published to the host. Full compose rewrites web and worker to hostname postgres. Host processes need a published Postgres.",
      ...
      "Services: postgres (durable state, unpublished), pr-agent-web (ROLE=web, POST /webhooks, GET /health, GET /ready), pr-agent-worker (queue consumers including code-index-build).",
      ...
      "GitHub needs HTTPS except when the webhook target is a loopback address. Compose publishes HTTP 7224 only. Put TLS in front, or run a smee-client or cloudflared process that forwards to /webhooks.",
      "A VPS plus Dokploy or Coolify is a TLS front for pr-agent-web. Route only the web service on internal port 7224. Leave Postgres unpublished. Keep DATABASE_URL on hostname postgres. Do not add a second compose file or expose the worker. Hetzner is the usual cheap 4 GB VPS. Hostinger, DigitalOcean, Vultr, Linode, and OVHcloud also work. See README Recommended hosts.",
