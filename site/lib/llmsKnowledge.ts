...
export const KNOWLEDGE_CHUNKS: readonly KnowledgeChunk[] = [
  ...
  },
  {
    ...
    body: lines([
      ...
      "/triage: apply-mode fix for open bot findings. /triage preview renders would-be diffs without push. /triage all applies the previewed set (refused without a matching preview on this head).",
      "/triage preview: render the would-be unified diff. No commits, no push.",
      "/triage all: apply the previewed set (optional exclude <thread ids>). Refused without a matching preview on this head.",
