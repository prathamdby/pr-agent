...
`.window` is the mock-up shell: `bg-surface`, `border-radius: var(--radius-md)`, `--shadow-float`, `will-change: transform`. `PrWindow` (`site/components/pr-window.tsx`) adds browser chrome: `border-b border-line bg-surface-raised px-3 py-2`, three `size-2.5 rounded-full bg-line` dots, and a URL pill `tabular rounded-xs bg-surface px-2.5 py-1 text-[11px] text-text-tertiary shadow-ring`. The PR mock is `aria-hidden`; the real examples live in the use-cases tabs.
