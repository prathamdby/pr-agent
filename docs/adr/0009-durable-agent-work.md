...
7. **Superseding and lanes** — Automated `synchronize` reviews use latest-head-wins semantics for the same PR. `/ask` uses a separate queue and reserved worker capacity. **Historical (pre-[ADR 0028](0028-orchestrated-review.md)):** general and security review lenses used separate singleton keys and progress comments (per PR/lens); new code writes only mode `review` with one active review slot per PR.
...
- `key_strict_fifo` can block a PR when a pg-boss job is failed; worker logs expose blocked keys and `docs/agent-work-ops.md` documents recovery. (Historical lens-scoped keys remain readable in old rows.)
