...
4. **Executor kind follows Phase 0.** Compiled workers (`import.meta.url` ends with `.js`) run cells on `worker_threads` so a stuck WASM module cannot pin the consumer event loop. TypeScript/vitest runs the same cell in-process so the interrupt stays observable. `worker.terminate()` was 2ms in Phase 0; child `SIGKILL` was 3ms. The regex input heuristic is removable because the interrupt stopped a catastrophic regex in 51ms.
