# ADR 0024: Workspace-primary grounding and evidence
...
head SHA, with GitHub `listFiles` as the anchor authority ([ADR 0012](0012-full-context-local-pr-workspace.md)).
...
prompts or repository content ([ADR 0023](0023-pi-native-agent-runtime.md)).

Cross-PR preference memory remains committed `.pr-agent/*.mdc` rules only
([ADR 0017](0017-mdc-repo-policy.md)). Optional navigation aids (ephemeral symbol
...
2. **Agent events are metadata-only.** Append-only `agent_events` rows record
   ...
   Reaffirm [ADR 0023](0023-pi-native-agent-runtime.md): no prompts, model text,
...
4. **Cross-PR finding history is threshold memory, not preference memory.**
   ...
   committed `.pr-agent/*.mdc` ([ADR 0017](0017-mdc-repo-policy.md)).
...
- Retention must cover event and code-index rows without expanding the ADR 0023
  audit boundary.

## Alternatives considered

- **RAG-primary grounding** — Rejected; indexes must not become publish authority.
- **Silent preference learning from dismissals** — Rejected; ADR 0017 owns prefs.
