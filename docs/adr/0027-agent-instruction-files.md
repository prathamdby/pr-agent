...
1. **Statically load** `AGENTS.md`, `CLAUDE.md`, and `GEMINI.md` from the PR head checkout root on every **orchestrated review run**. (Historical: once loaded for each independent review lens; lenses are retired — see [ADR 0028](0028-orchestrated-review.md).)
...
4. **Treat present files as binding** for that review: evidenced violations matching a specialist reporting gate are ordinary findings; prompts may cite these filenames when they appear in trusted context.
...
- Prompt cost budgets for every **orchestrated review run** grow by the shared guidance block.
