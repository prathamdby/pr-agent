...
1. Agents are read-only investigators. Review, ask, and verification investigate through one `execute({ code })` tool ([ADR 0033](0033-code-mode-execution.md)). Description and triage keep native workspace tools. Structured submit/publish tools stay native. No write or shell tools are enabled on review, ask, description, or verification sessions.
...
4. Auto and slash-command reviews get one stale-head reschedule. The worker may schedule one replacement for the latest head when automatic-review preflight or a publish gate detects that the PR head changed. The replacement preserves slash-command context when present. If the replacement also goes stale, it fails with retry guidance instead of rescheduling indefinitely.
