...
3. **Slash override.** `/review` always runs a full Review run regardless of file types.
...
- The unified automated review uses the same gate; docs-only pull requests still skip unless `/review` is invoked.
