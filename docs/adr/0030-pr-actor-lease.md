...
2. **Lease lifecycle on the runner.** Leased work types (review, description, triage, verification) acquire the lease before claiming the work item — a delivery waiting on the lease leaves its row `queued`, so queue-rank display and stale-queued diagnostics keep their meaning — renew it on `PR_ACTOR_LEASE_RENEWAL_INTERVAL_SECONDS` while the live holder still owns that epoch, and release it on completion, terminal failure, retry handoff, a failed claim, or a rejected payload load. Renewal matches `(resource_key, work_type, lease_epoch, work_item_id)`. A holder-clear from intake cancel fails the next renew without bumping the epoch. During execute, a separate observer watches durable skip or hold-loss on `REVIEW_CANCEL_POLL_INTERVAL_MS` (immediate first tick) and aborts the host signal; renewal stays the 120s health write, not the cancel channel. A failed SQL release leaves expiry as the recovery mechanism. Renewal is cooperative: a lost renewal logs at warn, aborts the execution signal, and prevents the next PR-surface mutation or durable checkpoint; it never marks the item failed for a lost lease.
