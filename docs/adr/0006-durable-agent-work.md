...
3. **Web/worker split** — `ROLE=web` serves `/health`, `/ready`, and `/webhooks`. `ROLE=worker` runs acknowledgement, CI-projection, review, ask, description, triage, verification, retention, and code-index-build workers from the same image.
