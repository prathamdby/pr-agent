...
- [ ] **Step 1: Write failing tests** for `format`/`renderReviewProgressComment` with `queuePosition: { position: 2, total: 10 }` → contains Queue + `#2 of 10`; without queuePosition → no Queue row; with tickState → no Queue row even if queuePosition passed; `#1 of 1` works.
- [ ] **Step 2: Write failing test** for `getReviewQueuePosition` (unit with mocked pool.query / queryOne, or smallest existing DB test helper) covering: mid-queue rank, sole item, null when status is running.
- [ ] **Step 3: Run targeted tests; confirm failure**
- [ ] **Step 4: Implement constants, lookup, render wiring**
- [ ] **Step 5: Run targeted tests; confirm pass**
- [ ] **Step 6: Update CONTEXT.md + docs/configuration.md + docs/operations.md in the same change**
...
- [ ] **Step 1: Extend ackExecutor test** — mock `getReviewQueuePosition` (or repository export) to return `{ position: 2, total: 10 }`; assert upserted body matches `/#2 of 10/` and Queue label.
- [ ] **Step 2: Extend ackExecutor test** — mock returns null; body has no Queue row / no `#… of …`.
- [ ] **Step 3: Implement wiring + warn-on-error**
- [ ] **Step 4: Run `npx vitest run test/ackExecutor.test.ts test/progressComment.test.ts test/reviewQueuePosition.test.ts` (or chosen test path); confirm pass**
...
- [ ] **Step 1: Run broader related tests if any renderCiSummary / publishReview coordination asserts exact queued table HTML**
- [ ] **Step 2: Fix any brittle exact-HTML expectations that omit the optional Queue row**
- [ ] **Step 3: deslop → commit → make-pr `--target main`**
