...
- **CI summary (optional):** when `ENABLE_REVIEW_CI_SUMMARY=true`, the progress stub and completed summary include a **CI** gate row for external checks on the PR head (excluding PR Agent’s own check). Passing CI gets a green notice; failing CI gets a short annotation/output digest with a fix hint. See [ADR 0024](adr/0024-review-ci-summary.md).
