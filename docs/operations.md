...
- **Review merge cancel:** when a PR is closed (merged or not), active review work is cancelled and the acknowledgement worker replaces the progress comment with `PR merged.` or `PR closed.` (same short layout, no progress table).
