...
- **Verification runs:** auto-triggered on `pull_request` `synchronize` (`VERIFICATION_AUTO_ACTIONS`, default `synchronize`; empty disables). Read-only: re-checks open bot inline findings against the new head, replies in each finding thread with a verdict (fixed, already-resolved, still-open, or dismissed), and resolves fixed threads. No ack reaction, progress comment, or summary comment is posted.
