...
- **Orchestrated review phase receipts** — `"review published"` also carries wall-clock phase fields when the orchestrated path ran: `recon_ms`, `specialist_correctness_ms`, `specialist_security_ms`, `specialist_quality_ms`, `specialist_tests_ms`, `specialists_parallel_ms`, `synthesis_ms`. Query p50/p95 by `model` / `provider` to size tripwires. All-empty (`findings_count=0`) runs still emit full phase receipts. Sequential phases should fit under `wall_clock_ms`; the sum of per-specialist durations may exceed wall clock because specialists run in parallel.
- **Duration tripwires** — Do not raise `QUEUE_EXPIRE_IN_SECONDS` or `REVIEW_SPECIALIST_TIMEOUT_MS` to “fix” one slow run without phase receipts. Specialist overruns must surface as progress **Failed** (not No findings) with budget attribution (`budgetKey`, `limitMs`, `usedMs` on the specialist failure). Prefer changing model depth or choice so healthy runs never graze the job deadline; leave defaults until measured healthy p95 sits comfortably below the tripwire.
