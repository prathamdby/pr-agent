...
- **`/loop`:** ack-only merge-loop briefing. No durable work item and no LLM. The acknowledgement worker reads the current head, the review progress or summary comment, open bot finding threads, and a lightweight CI snapshot, then replies under `## PR Agent Loop`. Copy is derived evidence (review currency, open P0/P1 and P2/P3, next mechanical action). It does not merge, approve, or change findings. See [ADR 0038](adr/0038-derived-merge-loop-briefing.md).
