...
- **Single active review:** one review may be queued or running per pull request. A duplicate `/review` is acknowledged but does not create another work item or change the active progress comment. `/review force` is the exception: it cancels the active run first (see below).
