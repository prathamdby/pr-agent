...
- **Pi-native agent runtime:** only **`AGENT_PROVIDER=pi`** (default) is supported. Set **`PI_PROVIDER`/`PI_MODEL`** for general sessions, optional **`PI_ORCHESTRATOR_*`** and **`PI_FALLBACK_*`**, and an optional **`models.json`** catalog. See [ADR 0031](adr/0031-pi-native-agent-runtime.md).
