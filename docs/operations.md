...
- **Orchestrated reviews:** the server builds the specialist brief from PR intent, changed paths, risk-path flags, and loaded repository context. The progress stub marks Recon Done as soon as that deterministic brief is ready. Before specialists start, the server runs a changed-file read pass into the evidence ledger (respecting workspace size/truncation caps). Correctness, security, quality, and tests specialists then run in parallel. Evidence-valid reports publish through the deterministic finding path without one serial judgment turn per specialist. The server renders the final summary from the finding ledger and run state. Specialist ticks use shared status copy (`Waiting`, `Running`, `No findings`, `N findings`, `Failed`); `N findings` counts accepted ledger placements (inline plus summary-only), not inline threads alone. Full coverage requires the file pass (or capped-read receipts) for every changed path plus no specialist/budget failure. Partial coverage notes on the PR stay short (named specialist failure, budget key/limit/used, optional unread count); full inspected/skipped path inventories go to worker logs and `"review published"` analytics only. `no_findings` means no qualifying findings within inspected coverage.
...
- **Pi-native agent runtime:** Set **`PI_PROVIDER`/`PI_MODEL`** for model sessions, optional **`PI_FALLBACK_*`**, and an optional **`models.json`** catalog. Normal review bookends are server-owned and do not create an orchestrator model session. See [ADR 0031](adr/0031-pi-native-agent-runtime.md).
...
- **Orchestrated review receipts** — `"review published"` carries `active_budget_ms`, `active_ms`, `active_overrun_ms`, specialist and finalization phase times, timeout and partial-coverage markers, prompt profile, and inspected/changed path counts. Queue wait is excluded because the active clock starts at orchestrator entry. Successful normal runs must have `active_overrun_ms=0`.
- **Duration tripwires** — Normal review has a 15-minute active SLA and an eight-minute specialist cap. Do not raise `QUEUE_EXPIRE_IN_SECONDS` or `REVIEW_SPECIALIST_TIMEOUT_MS` to chase latency. A hit must surface as **Failed**, never No findings, with `budgetKey`, `limitMs`, and `usedMs` in the short final partial-coverage note (unread count only when paths remain unread). Full path inventories stay in logs and analytics. Exhaustive or deep review, if added later, must use a separate mode without the normal SLA.
