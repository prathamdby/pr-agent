...
- **PostHog (optional):** knobs `POSTHOG_PROJECT_TOKEN`, `POSTHOG_HOST`, `POSTHOG_ENABLED`, and `POSTHOG_EXCEPTION_AUTOCAPTURE` load through `loadConfig()` ([configuration.md](configuration.md)). The client is always constructed at bootstrap (including when the token is empty). When enabled, events use distinct ids `installation:<id>`, `worker`, or `server`, plus coarse properties such as owner/repo/PR number, work-item id, provider, event name, and error messages—not raw diffs or finding bodies. Exception autocapture (default on) may send uncaught exception payloads to the configured host. Leave the token empty (or set `POSTHOG_ENABLED=false`) to keep outbound analytics off.
