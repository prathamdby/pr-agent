...
- On **`pull_request`** (`opened`, `synchronize`, `reopened`), durable intake runs. On **`opened`**, enqueues an automated **orchestrated review run** and a **description run** when `FEATURE_REVIEW` / `FEATURE_DESCRIBE` are `auto`. On **`synchronize`**, enqueues a **verification run** that re-checks open bot inline findings against the new head (`FEATURE_VERIFICATION`; `off` disables). **`reopened`** does not auto-enqueue work. Review workers add an acknowledgement reaction on the PR issue, post a **review progress comment** stub, run the **review orchestrator** (recon → brief → four **specialists** → incremental **thread batches** → synthesis), tick the stub as each specialist resolves, and upsert **`## PR Agent Review`** on the **PR conversation** when the summary publishes. Description workers merge generated content into the PR body under **`## PR Agent Description`**, preserving user-authored text above that header (by default the PR title is not overwritten). Inline P0 to P2 threads land as **thread batches** during the run; the Files-tab **review pointer body** includes a collapsible **agent fix prompt** aggregating accepted findings.
- On **`issue_comment`** and **`pull_request_review_comment`** (`created` only), detects `/help`, `/ask`, `/describe`, `/review`, and `/triage`, and also promotes `@bot` mentions (same association allowlist as slash) to ask intake. Enqueues **agent work items** and routes commands. **PR-surface I/O** (reactions, replies, reviews) is published by workers, not on the webhook fiber.
...
- **Review superseding:** when a newer automated **orchestrated review run** is enqueued for the same PR, it supersedes queued auto-reviews and requests cooperative cancel on an in-flight auto-review. Automated **verification** on `synchronize` uses the same supersede/cancel pattern for prior verification items. Slash-command reviews are not superseded.
- **Orchestrated review:** one **review orchestrator** session (recon, brief, judgment, synthesis) plus four parallel **specialists** (correctness, security, quality, tests). Orchestrator tool rounds use **`MAX_TOOL_ROUNDS`** for recon and **`ORCHESTRATOR_JUDGMENT_MAX_TOOL_ROUNDS`** per judgment turn. Specialists submit **specialist reports** (`submit_findings_report`); the orchestrator publishes **thread batches** via `publish_thread` and the final summary via `publish_summary`. If the summary never publishes, publish-recovery / deterministic degradation may still land a summary; otherwise a plain-text fallback notice may be posted.
- **Review pointer link:** on the second and later **orchestrated review runs** for a PR, the Files-tab pointer links to the existing **review summary comment** when it can be verified; the first completed summary uses plain text only.
- **Merge verdict:** the **review summary comment** includes a **Merge verdict** row with a model score (/5) and one-sentence rationale. When P0/P1 findings are open, validation clamps the score to ≤3 and rejects safe-to-merge wording; when the model omits the verdict, a mechanical fallback is shown.
...
- **Tool surface:** production review and ask agents use the local workspace tools `listChangedFiles`, `readWorkspaceFile`, `searchWorkspace`, `getWorkspaceDiff`, and `getWorkspaceBlame`, plus 2 Context7 doc tools. GitHub reads happen server-side during preflight and publish. The **review orchestrator** publishes via `publish_thread` / `publish_summary`; specialists only call `submit_findings_report`. See [ADR 0004](adr/0004-native-pi-ai-toolset.md) and [ADR 0028](adr/0028-orchestrated-review.md).
...
- **Cursor provider:** set **`AGENT_PROVIDER=cursor`**, **`CURSOR_API_KEY`**, and **`PI_MODEL`** (e.g. `composer-2.5`). Worker registers pi-ai api `cursor-sdk` and runs Cursor local agents with an HTTP MCP bridge to pr-agent's workspace, Context7, and review publish tools. See [ADR 0013](adr/0013-cursor-sdk-provider.md).
...
- **Lightweight review completion:** automated reviews on docs-only trivial PRs may finish without a full **orchestrated review run** (**trivial change exemption**). See [ADR 0014](adr/0014-lightweight-review-completion.md).
