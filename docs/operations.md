...
- **PostHog (optional):** knobs `POSTHOG_PROJECT_TOKEN`, `POSTHOG_HOST`, `POSTHOG_ENABLED`, and `POSTHOG_EXCEPTION_AUTOCAPTURE` load through `loadConfig()` ([configuration.md](configuration.md)). The client is always constructed at bootstrap (including when the token is empty). When `POSTHOG_ENABLED` is empty/unset, enablement follows whether the token is non-empty. When enabled, events use distinct ids `installation:<id>`, `worker`, or `server`, plus coarse operational properties (owner/repo/PR number, optional work-item id, provider/model labels, event names, durations, delivery ids, error messages, and similar counters)—not raw diffs or finding bodies. Exception autocapture (default on) may send uncaught exception payloads to the configured host. Set `POSTHOG_ENABLED=false` to no-op `capture` / `captureException` (or leave the token empty for the derived-off default).
