...
- **`/cancel`:** cancels the active review for the PR (auto or slash, queued or running). The acknowledgement worker edits the **review progress comment** in place to `Cancelled by @login.` while keeping Head/Source/(CI)/Recon/specialist table rows. No review is active → short reply only.
