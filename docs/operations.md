...
| `nub run typecheck`                    | `tsc --noEmit` for `src/` + `test/`, then `tools/oxlint/`               |
...
Type awareness comes from [`.oxlintrc.json`](../.oxlintrc.json) `options.typeAware` (lint scripts do not pass `--type-aware`). Keep `nub run typecheck` as separate `tsc`. Type-aware lint requires `oxlint-tsgolint` (dev dependency). The local anti-slop jsPlugin lives at [`tools/oxlint/anti-slop`](../tools/oxlint/anti-slop). [`.oxlintrc.json`](../.oxlintrc.json) enables the corpus-clean rules as errors everywhere (`no-object-parameters`, `no-reflect-apply`, `no-reflect-get`, `no-shape-in-symbol-names`, `no-unknown-type-aliases`, `no-widen-then-assert`). The remaining anti-slop rules are errors only under `tools/oxlint/**` until the existing `src/` / `test/` / `site/` corpus is migrated. Registry cooling-window settings live only in [`nub.jsonc`](../nub.jsonc) (`install.minimumReleaseAge`, `install.minimumReleaseAgeExclude`); edit that file when adding or removing temporary excludes.
