...
- **Reviewer fan-out:** every full Review run creates eight independent read-only reviewer sessions for correctness, security, tests, maintainability, project standards, reliability, API/contracts, and adversarial analysis. They share one prepared local PR workspace but have isolated session state and cannot publish. At most **`REVIEW_AGENT_CONCURRENCY`** reviewer sessions run simultaneously (default `4`).
- **Required coverage and validation:** correctness and security reports are required; if either fails, the Review run fails without synthesis. Other reviewer failures are reported to the orchestrator as degraded coverage. Candidate P0/P1 findings receive an independent validator pass before synthesis; rejected candidates are omitted.
- **Review synthesis and publish:** one orchestrator session receives the reviewer reports, degraded-coverage state, and read-only tools. It is the only session with **`submitReview`** and must produce one complete ReviewPayload. Its tool loop is capped at **`MAX_TOOL_ROUNDS`**. If **`submitReview`** never succeeds, publish-recovery nudges run up to **`MAX_REVIEW_PUBLISH_ATTEMPTS`**, then a plain-text fallback comment may be posted when structured publish is exhausted.
