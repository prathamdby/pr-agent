...
- **Orchestrated reviews:** the server builds the specialist brief from PR intent, changed paths, risk-path flags, and loaded repository context. The progress stub marks Recon Done as soon as that deterministic brief is ready. Correctness, security, quality, and tests specialists then run in parallel. Evidence-valid reports publish through the deterministic finding path without one serial judgment turn per specialist. The server renders the final summary from the finding ledger and run state. Specialist ticks use shared status copy (`Waiting`, `Running`, `No findings`, `N findings`, `Failed`); `N findings` counts accepted ledger placements (inline plus summary-only), not inline threads alone. A budget failure names the key, limit, and used time and produces explicit partial coverage. `no_findings` means no qualifying findings within inspected coverage.
