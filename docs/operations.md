...
- **Vercel** site deploys install Nub in [`site/vercel.json`](../site/vercel.json) (`install.sh` + `nub ci`), then build with `nub run build`.
...
Type awareness comes from [`.oxlintrc.json`](../.oxlintrc.json) `options.typeAware` (lint scripts do not pass `--type-aware`). Keep `nub run typecheck` as separate `tsc`. Type-aware lint requires `oxlint-tsgolint` (dev dependency). [`nub.jsonc`](../nub.jsonc) `install.minimumReleaseAge` is `"7d"` for registry installs, with temporary `install.minimumReleaseAgeExclude` entries: Oxc (`oxlint@1.75.0`, `oxlint-tsgolint@7.0.2001`, `@oxlint/*`, `@oxlint-tsgolint/*`) — remove after 2026-07-29; Pi 0.82.1 (`@earendil-works/pi-*@0.82.1`) plus `evlog@2.22.3`, `oxfmt@0.60.0` / `@oxfmt/*`, `pg-boss@12.26.3`, `posthog-node@5.46.1` / `@posthog/*` — remove after 2026-08-01; `brace-expansion@5.0.8` (GHSA-mh99-v99m-4gvg override in `package.json`) — remove override + exclude after 2026-07-30 once upstream ages past the gate.
