...
| Pi session seam                  | `src/agent/runtime/`                    | `piSession.ts` (`createPiSession`, `createFakePiSession`); `createFeaturePiSession` resolves the attempt's model and wraps `send` so sessions keep checkpoint/snapshot persistence — feature harnesses must not import raw Pi SDK sessions |
...
| Code Mode                        | `src/agent/codemode/`                   | `execute({ code })` AST interpreter; `tools.*` delegates to canonical workspace capabilities; terminal submit/publish tools stay native siblings                                                                                           |
