...
| Thread-reply classify       | `THREAD_REPLY_CLASSIFY_CONCURRENCY`         | `2`                         | pg-boss thread-reply classification workers (implicit bot-thread asks when `ENABLE_THREAD_REPLIES=true`)                                                                                                                                                                                                                                                   |
...
| PostHog project token       | `POSTHOG_PROJECT_TOKEN`                     | empty                       | optional analytics; empty disables capture                                                                                                                                                                                                                                                                                                                 |
| PostHog host                | `POSTHOG_HOST`                              | empty                       | optional host override; empty uses posthog-node default                                                                                                                                                                                                                                                                                                    |
| Label effort                | `ENABLE_REVIEW_LABELS_EFFORT`               | `true`                      |                                                                                                                                                                                                                                                                                                                                                            |
| Label security              | `ENABLE_REVIEW_LABELS_SECURITY`             | `false`                     |                                                                                                                                                                                                                                                                                                                                                            |
| Thread replies              | `ENABLE_THREAD_REPLIES`                     | `false`                     | enqueue async thread-reply classification when a maintainer replies in a bot inline review thread (no slash command); workers promote to `/ask`                                                                                                                                                                                                            |
| Review commit status        | `ENABLE_REVIEW_COMMIT_STATUS`               | `false`                     | post GitHub commit status `pr-agent/review` on the PR head after summary publish; uses the same P0/P1/P2 failure rule as the review check run. Use this context in branch protection rules.                                                                                                                                                                |
| Review CI summary           | `ENABLE_REVIEW_CI_SUMMARY`                  | `false`                     | add a **CI** gate row on the review progress stub and completed review summary: green when external checks pass, or a short failure digest (reason + fix hint from check annotations/output) when they fail; excludes PR Agent’s own check run; soft-fails if Checks permission is missing                                                                 |
| CI summary wait             | `REVIEW_CI_SUMMARY_WAIT_MS`                 | `15000`                     | max time publish may poll for external checks to leave pending before summarizing (0 disables wait)                                                                                                                                                                                                                                                        |
| CI summary poll interval    | `REVIEW_CI_SUMMARY_WAIT_POLL_MS`            | `2000`                      | poll interval while waiting for terminal external CI                                                                                                                                                                                                                                                                                                       |
| CI summary max failures     | `REVIEW_CI_SUMMARY_MAX_FAILURES`            | `3`                         | max failing checks/statuses to dig into with annotations for the digest                                                                                                                                                                                                                                                                                    |
...
| `THREAD_REPLY_CLASSIFY_QUEUE`              | `agent-work-thread-classify`                                                                                                                                                |
...
| `THREAD_REPLY_CLASSIFY_DEAD_LETTER_QUEUE`  | `agent-work-thread-classify-dead`                                                                                                                                           |
...
| `REVIEW_CI_SUMMARY_WAIT_POLL_MS` / `REVIEW_CI_SUMMARY_MAX_FAILURES`                                                  | 2000 / 3 — code fallbacks for the optional CI summary gate (env `REVIEW_CI_SUMMARY_*` overrides at config load)                                                             |
