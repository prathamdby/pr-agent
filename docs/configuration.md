...
| `CI_REFRESH_QUEUE`                         | `agent-work-ci-refresh` — leftover intake from `workflow_run` or `check_suite` completed. The worker shim enqueues one `ci-projection` and exits                                                                                 |
| `CI_PROJECTION_QUEUE`                      | `agent-work-ci-projection` — worker renders CI cells from `pr_head_ci_state`. Intake and later writers send a 5s debounced job per head                                                                                          |
| `CI_REFRESH_RETRY_DELAY_SECONDS`           | 15, leftover refresh-intake singleton window. Same-head `ci-refresh` sends join that slot                                                                                                                                        |
