...
| Review tool rounds fallback | `MAX_TOOL_ROUNDS`                           | `24`                        | shared fallback when role-specific review tool-round envs are unset                                                                                                                                                                                                                                                                                        |
| Reviewer tool rounds        | `MAX_TOOL_ROUNDS_REVIEWER`                  | `MAX_TOOL_ROUNDS` / `24`    | tool-loop cap for each reviewer-agent session                                                                                                                                                                                                                                                                                                              |
| Validator tool rounds       | `MAX_TOOL_ROUNDS_VALIDATOR`                 | `MAX_TOOL_ROUNDS` / `24`    | tool-loop cap for each high-risk validator session                                                                                                                                                                                                                                                                                                         |
| Orchestrator tool rounds    | `MAX_TOOL_ROUNDS_ORCHESTRATOR`              | `MAX_TOOL_ROUNDS` / `24`    | tool-loop cap for the Review orchestrator investigation phase                                                                                                                                                                                                                                                                                              |
...
| Review worker concurrency   | `REVIEW_CONCURRENCY`                        | `2`                         | concurrent durable Review jobs per worker process; each job has its own bounded inner agent fan-out                                                                                                                                                                                                                                                        |
| Reviewer fan-out            | `REVIEW_AGENT_CONCURRENCY`                  | `4`                         | max concurrent reviewer-agent sessions inside one Review run; provider pressure ≈ `REVIEW_CONCURRENCY × REVIEW_AGENT_CONCURRENCY` per worker                                                                                                                                                                                                               |
| Validation candidate cap    | `REVIEW_VALIDATION_MAX_CANDIDATES`          | `16`                        | max P0/P1 candidates independently validated per Review run; overflow stays unvalidated and is reported as degraded coverage                                                                                                                                                                                                                               |
...
| Verification tool rounds    | `MAX_TOOL_ROUNDS_VERIFICATION`              | `32`                        | agent investigation cap for verification runs                                                                                                                                                                                                                                                                                                              |
...
| PostHog project token       | `POSTHOG_PROJECT_TOKEN`                     | empty                       | optional project API key; with `POSTHOG_ENABLED=true` an empty token still constructs a client (SDK no-ops outbound)                                                                                                                                                                                                                                       |
| PostHog host                | `POSTHOG_HOST`                              | empty                       | optional PostHog API host (omit / empty uses the SDK default)                                                                                                                                                                                                                                                                                              |
| PostHog enabled             | `POSTHOG_ENABLED`                           | `true`                      | when `false`, skip client construction entirely; default `true` preserves prior empty-token construction                                                                                                                                                                                                                                                   |
| PostHog exception autocapture | `POSTHOG_EXCEPTION_AUTOCAPTURE`           | `true`                      | when enabled, PostHog may capture unhandled exceptions / `captureException` payloads to the configured host                                                                                                                                                                                                                                                |
...
| Review check run            | `ENABLE_REVIEW_CHECK_RUN`                   | `false`                     | post GitHub check run `PR Agent Review` (or lens-specific name) on the PR head; starts `in_progress`, then completes `failure` for P0/P1 findings and `success` otherwise. Requires GitHub App Checks read/write permission. Use the check name in branch protection rules.                                                                                |
...
| Verification auto actions   | `VERIFICATION_AUTO_ACTIONS`                 | `synchronize`               | comma-separated `pull_request` actions that auto-trigger verification runs; empty string disables                                                                                                                                                                                                                                                          |
...
When PostHog is enabled with a project token, events leave the process to the configured host. **Distinct ids:** `server` (webhook intake), `worker` (provider boot / Cursor worker), `installation:<id>` (work-item / review / ask / description / triage analytics). **Typical properties** (non-exhaustive): GitHub event name and delivery id; owner/repo/PR number; work-item type; provider/model; publish attempt counts; finding counts/severities; error messages on failure paths. Exception autocapture and `captureException` may send error objects and related properties.
...
| Symbol                                                               | Role                                                                                                   |
| -------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| `REVIEW_SUMMARY_SENTINEL`                                            | Unified PR conversation summary marker                                                                 |
...
| `instructions`     | string  | 1000 chars  | Extra instructions applied to the unified Review run                              |
| `pathInstructions` | array   | ≤20 entries | Glob `path` (200 chars) + `instructions` (1000 chars); matched changed files only |
| `lensOverrides`    | any     | deprecated  | Accepted for migration compatibility, ignored with a warning under unified Review |
...
severityFloor: 2
instructions: Flag new outbound HTTP calls that do not set a timeout.
pathInstructions:
  - path: "src/auth/**"
