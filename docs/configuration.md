...
| Specialist timeout        | `REVIEW_SPECIALIST_TIMEOUT_MS`     | `900000`                    | **Tripwire:** total wall-clock budget for one specialist (jitter, retries, validation repair). Do not raise after one slow run without phase receipts on `review published` (`recon_ms`, `specialist_*_ms`, `specialists_parallel_ms`, `synthesis_ms`). Size from healthy p95 + margin. A specialist that exceeds this budget must tick **Failed** (never clean No findings). |
...
**Tripwire policy (receipts first):** `QUEUE_EXPIRE_IN_SECONDS` and `REVIEW_SPECIALIST_TIMEOUT_MS` are tripwires, not load-bearing knobs. After a slow or empty long run, read phase fields on `"review published"` (`recon_ms`, `specialist_correctness_ms`, `specialist_security_ms`, `specialist_quality_ms`, `specialist_tests_ms`, `specialists_parallel_ms`, `synthesis_ms`) plus existing `wall_clock_ms` / `generation_ms` / `model` / `provider`. Size tripwires from healthy p50/p95 + margin. If a healthy run hits a tripwire, remeasure and update the receipt-backed limit; do not silently widen defaults after one incident. Pathological runs must hit a **named** limit (specialist Failed names `budgetKey` / `limitMs` / `usedMs`).
