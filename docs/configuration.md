...
| Specialist attempt timeout | `REVIEW_SPECIALIST_TIMEOUT_MS`     | `900000`                    | per-specialist wall-clock cap (orchestrator V2); effective timeout is `min(config, remaining budget minus start stagger)`; timeout aborts then disposes the session, retries only while budget/deadline remains                      |
...
Review check runs are always on: the worker posts GitHub check run **`PR Agent Review`** on the PR head, starts `in_progress`, then completes with a findings-derived conclusion when all four specialists finished, or **`neutral`** when coverage is partial (any specialist failed). Optional commit status `pr-agent/review` maps success → `success`, findings failure → `failure`, and neutral/cancelled → `error`. Requires GitHub App Checks read/write; soft-fails if that permission is missing.

**Operator callout — branch protection:** per-lens check runs (`PR Agent Security Review`, `PR Agent Quality Review`, `PR Agent Tests Review`) no longer exist. Update branch protection rules that referenced those names to require **`PR Agent Review`** only. A green `PR Agent Review` check always means all four specialists ran.

| Symbol                                                                                                                 | Role                                                                                                                                                           |
| ---------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `REVIEW_SUMMARY_SENTINEL`                                                                                              | PR conversation summary marker (`## PR Agent Review`)                                                                                                          |
| `LEGACY_REVIEW_SUMMARY_SENTINELS` / `LEGACY_REVIEW_POINTER_BODIES`                                                     | Historical security/quality/tests markers in `legacyReviewLenses.ts` — recognition only; never written by new code                                             |
...
| `MAX_THREAD_PUBLISH_CALLS`                                                                                             | 8 judgment publish attempts per run; counts empty-after-suppression and failed GitHub attempts, not only durable successes; later findings are summary-only    |
| `MAX_SPECIALIST_FINDINGS`                                                                                              | 20 findings cap per specialist report                                                                                                                          |
| `ORCHESTRATOR_JUDGMENT_MAX_TOOL_ROUNDS`                                                                                | 4 tool rounds per judgment turn (one `publish_thread` plus repair)                                                                                             |
| `RUN_DEADLINE_BUDGET_FRACTION`                                                                                         | 0.8 of `queueExpireInSeconds` — hard orchestrated-run deadline so the handler returns before pg-boss expires                                                   |
| `ORCHESTRATOR_SEND_ABORT_POLL_MS`                                                                                      | 250 ms — bounded cheap cancel poll (`shouldCancelRun`) for the single run-scoped monitor (recon through summary); never polls the full stale-head publish gate |
| `SPECIALIST_DISPATCH_STAGGER_MS`                                                                                       | 2000 ms per specialist index — deterministic spawn stagger to avoid a five-way provider burst                                                                  |
| `JUDGMENT_DEGRADED_NOTE`                                                                                               | Maintainer-visible note when judgment/synthesis degrades to deterministic publish                                                                              |
| `RUN_DEADLINE_NOTE`                                                                                                    | Maintainer-visible note when the internal run time budget alone forces deterministic summary publish (not judgment degradation)                                |
...
Legacy `.pr-agent.yml` is ignored. Rules augment prompts only. They never replace the structured publish path (`publish_thread` / `publish_summary`) or change output schemas.
...
Prompt prose (investigator contracts) remains in `src/review/prompts/`, `src/review/orchestrator/prompts/`, `src/agent/prompts/`, `src/agent/ask/`, and `src/agent/description/`.
