...
| PostHog project token       | `POSTHOG_PROJECT_TOKEN`                     | empty                       | optional; empty token still constructs the client (intentional). Useful capture requires a non-empty token. See [Data Privacy](../README.md#data-privacy) / [operations security](operations.md#security) for distinct ids and properties sent when enabled                                                                                              |
...
| PostHog enabled             | `POSTHOG_ENABLED`                           | token-derived               | when unset, `true` iff `POSTHOG_PROJECT_TOKEN` is non-empty; set explicitly to override. Empty-token client construction remains intentional either way                                                                                                                                                                                                     |
