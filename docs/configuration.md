...
**Tripwire policy (receipts first):** `QUEUE_EXPIRE_IN_SECONDS` and `REVIEW_SPECIALIST_TIMEOUT_MS` are tripwires, not latency fixes. Do not raise either to chase the normal SLA. Read `active_budget_ms`, `active_ms`, `active_overrun_ms`, phase fields, timeout markers, and inspected/changed path counts on `"review published"`. A hit must name `budgetKey`, `limitMs`, and `usedMs` in progress and final partial-coverage output.
