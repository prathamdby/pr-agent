...
| Review tool rounds fallback | `MAX_TOOL_ROUNDS`                           | `16`                        | shared fallback when role-specific review tool-round envs are unset                                                                                                                                                                                                                                                                                        |
...
| PostHog project token       | `POSTHOG_PROJECT_TOKEN`                     | empty                       | optional; empty token still constructs the client (intentional). Capture delivery still needs enablement (token-derived or `POSTHOG_ENABLED=true`) plus a usable token. See [Data Privacy](../README.md#data-privacy) / [operations security](operations.md#security)                                                                                                                                                  |
| PostHog host                | `POSTHOG_HOST`                              | empty                       | optional PostHog API host; empty omits host so posthog-node uses its default cloud endpoint                                                                                                                                                                                                                                                                |
| PostHog enabled             | `POSTHOG_ENABLED`                           | token-derived               | when empty/unset, `true` iff `POSTHOG_PROJECT_TOKEN` is non-empty (else `false`); set `true`/`false` to override. When `false`, `capture` / `captureException` are no-ops (client still constructed)                                                                                                                                                        |
| PostHog exception autocapture | `POSTHOG_EXCEPTION_AUTOCAPTURE`           | `true`                      | when `true`, posthog-node may send uncaught exception payloads to the configured host                                                                                                                                                                                                                                                                      |
