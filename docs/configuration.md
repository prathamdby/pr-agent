...
Review check runs are always on. The acknowledgement worker posts `PR Agent Review` on the PR head and starts it as `in_progress`. Full-coverage runs complete with `failure` for any P0/P1/P2 finding and `success` when findings are empty or P3-only. Partial specialist coverage completes as `neutral`; the optional commit status reports `error`. Slash `/cancel`, close/merge cancel, and worker-observed cancel complete the check as `cancelled` (idempotent if already finished). Checks require GitHub App read/write permission and soft-fail when that permission is missing.
...
| `reviewProgressCancelledNote` / `sanitizeGithubLogin` / `ReviewCancelAttribution`                                      | Cancelled progress stub notice (`Cancelled by @login…`, `PR merged.`, or `PR closed.`) and login sanitizer |
