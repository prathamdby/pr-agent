...
| CI summary wait             | `REVIEW_CI_SUMMARY_WAIT_MS`                 | `15000`                     | max time publish may poll for external checks to leave pending before summarizing the always-on **CI** gate row (0 disables wait); green when checks pass, short failure digest when they fail; excludes PR Agent’s own check; soft-fails if Checks permission is missing                                                                                  |
