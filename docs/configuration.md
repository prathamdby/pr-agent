...
| Specialist timeout        | `REVIEW_SPECIALIST_TIMEOUT_MS`     | `480000`                    | **Tripwire:** total wall-clock budget for one specialist (jitter, retries, validation repair), capped at eight minutes by `REVIEW_SPECIALIST_BUDGET_MS`. The cap is based on the measured 7.9-minute longest healthy completion. Higher env values do not widen normal reviews. A specialist that exceeds this budget must tick **Failed** with the named budget receipt, never clean No findings. |
...
| Job expire                | `QUEUE_EXPIRE_IN_SECONDS`          | `3600`                      | **Tripwire:** hard job ceiling (orchestrated model window ≈ expire × 0.8 − finalization). Do not raise for a single slow review without phase receipts; widen only after measured healthy p95 proves the tripwire sits below good runs.                                                                                                                                                            |
...
| `REVIEW_ACTIVE_BUDGET_MS`                                                                                              | 900000 — hard 15-minute normal-review active budget from orchestrator entry                    |
| `REVIEW_SPECIALIST_BUDGET_MS`                                                                                          | 480000 — hard eight-minute cap for one normal-review specialist                                |
...
Normal orchestrated reviews have a hard active-compute budget of `REVIEW_ACTIVE_BUDGET_MS` (`900000`, 15 minutes) from orchestrator entry through terminal publication. Model work stops `REVIEW_FINALIZATION_WINDOW_MS` before the smaller of that active deadline and the pg-boss return deadline. Each specialist attempt uses the smallest of `REVIEW_SPECIALIST_TIMEOUT_MS`, the remaining active window, and the remaining job window. Queue wait before orchestrator entry does not consume the active budget.

**Tripwire policy (receipts first):** `QUEUE_EXPIRE_IN_SECONDS` and `REVIEW_SPECIALIST_TIMEOUT_MS` are tripwires, not latency fixes. Do not raise either to chase the normal SLA. Read `active_budget_ms`, `active_ms`, `active_overrun_ms`, phase fields, timeout markers, and inspected/changed/skipped path counts (plus capped unread path sample) on `"review published"`. A hit must name `budgetKey`, `limitMs`, and `usedMs` in progress and final partial-coverage output. Path inventories stay off the PR summary body.

**Changed-file read pass:** before specialists start, the server attempts a workspace read of every changed path into the evidence ledger (`server_changed_file_pass`), within `LOCAL_WORKSPACE_MAX_FILE_BYTES` / `LOCAL_WORKSPACE_READ_RESPONSE_BYTES`. Bounded failures (oversize, binary, deleted, missing from checkout) still count as attempted under workspace rules. An active-budget stop mid-pass marks coverage partial and skips specialists.

| Symbol                                   | Default / role                                                 |
| ---------------------------------------- | -------------------------------------------------------------- |
| `MAX_TOOL_ROUNDS`                        | 24 for specialist investigation                                |
