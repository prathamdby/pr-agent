...
| `PR_ACTOR_LEASE_DEFER_SECONDS`             | 15, `startAfter` delay for the deferred redelivery a lease-blocked delivery arms                                                                                    |
...
| `reviewProgressCancelledNote` / `sanitizeGithubLogin` / `ReviewCancelAttribution`                                      | Cancelled progress stub notice (`Cancelled by @login…` or `PR merged.`) and login sanitizer    |
