...
| `MAX_THREAD_PUBLISH_CALLS`                                                                                             | 8 incremental inline batches per review run; later findings remain summary-only                                                                                                   |
...
| `ORCHESTRATOR_SEND_ABORT_POLL_MS`                                                                                      | 250 ms — bounded cheap cancel poll (`shouldCancelRun`) while an orchestrator `session.send` is in flight or specialists are pending; never polls the full stale-head publish gate |
