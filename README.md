...
- **`PI_MODEL`**: Cursor model id. Supported values include `composer-2.5`, `composer-2`, `gpt-5.5`, `claude-opus-4-7`, and `auto` (see [`src/agent/providers/cursor/models.ts`](src/agent/providers/cursor/models.ts)). Append `-fast` for the fast tier when the model supports it (for example `composer-2.5-fast`, `gpt-5.5-fast`). The worker discovers fast-capable models from `Cursor.models.list()` at boot. Plain ids use the standard tier.
