...
4. Repository permissions (typical): **Issues** and **Pull requests** read/write, **Contents** read/write, **Metadata** read, **Checks** read/write. Contents write is only needed for `/triage`. **Commit statuses** read/write when `ENABLE_REVIEW_COMMIT_STATUS=true`. Checks read also powers the optional **CI summary** gate (`ENABLE_REVIEW_CI_SUMMARY`).
